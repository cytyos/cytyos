...
export const AdminPage = () => {
  ...
  const loadEvents = async () => {
    ...
    // Puxa as últimas 50 ações do banco
  ...
  return (
    <div className="min-h-screen bg-[#0f111a] p-8 text-white">
      <div className="max-w-5xl mx-auto space-y-8">
                ...
                <div className="bg-gray-900/50 border border-gray-800 p-6 rounded-2xl shadow-xl">
                    ...
                    <div className="flex flex-col md:flex-row gap-4 items-end">
                        <div className="w-full md:flex-1">
                            <label className="text-xs text-gray-500 font-bold ml-1 mb-1 block">CÓDIGO</label>
                            <input 
                                value={newCode}
                        ...
                        <div className="w-full md:w-48">
                            <label className="text-xs text-gray-500 font-bold ml-1 mb-1 block">DURAÇÃO</label>
                            <select 
                                value={duration} 
                            ...
                            <div key={coupon.id} className="bg-gray-800/40 border border-gray-700/50 rounded-xl overflow-hidden transition hover:border-indigo-500/30">
                                <div className="p-4 flex items-center justify-between">
                                    <div className="flex items-center gap-4">
                                        <div className="h-10 w-10 rounded-full bg-indigo-500/10 flex items-center justify-center text-indigo-400 font-bold"><Tag size={18} /></div>
                                        <div>
                                    ...
                                    <div className="flex items-center gap-3">
                                         <button onClick={() => toggleLeads(coupon.code)} className={`text-xs font-bold px-3 py-1.5 rounded-lg border transition flex items-center gap-2 ${expandedCoupon === coupon.code ? 'bg-indigo-600 border-indigo-500 text-white' : 'bg-gray-800 border-gray-700 text-gray-400 hover:text-white'}`}>
                                            <Users size={14} /> Ver Leads {expandedCoupon === coupon.code ? <ChevronUp size={14}/> : <ChevronDown size={14}/>}
                                         </button>
        ...
        {/* === TAB 2: RADAR (Só a tabela, sem gráficos) === */}
        {activeTab === 'radar' && (
            <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                <div className="flex justify-between items-center">
                    <h2 className="text-xs font-bold text-gray-500 uppercase tracking-wider ml-1">Últimas 50 Atividades</h2>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-gray-800/50 text-xs text-gray-400 border-b border-gray-700">
                                <th className="p-4 font-bold uppercase w-12">Tipo</th>
                                <th className="p-4 font-bold uppercase">Usuário</th>
                                <th className="p-4 font-bold uppercase">Ação</th>
                                <th className="p-4 font-bold uppercase">Detalhes (JSON)</th>
                                <th className="p-4 font-bold uppercase text-right">Hora</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-800">
                            {events.map((ev) => (
                                <tr key={ev.id} className="hover:bg-white/5 transition-colors">
                                    ...
                                    <td className="p-4">
                                        <div className="text-sm font-bold text-white">{ev.user_email || 'Anônimo'}</div>
                                    </td>
                                    <td className="p-4">
                            ...
                            {events.length === 0 && !loadingEvents && (
                                <tr>
                                    <td colSpan={5} className="p-8 text-center text-gray-500 italic">
                                        Nenhuma atividade registrada ainda. Use o app para testar!
                                    </td>
                                </tr>
                            )}
                        </tbody>
