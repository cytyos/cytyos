...
export const AdminPage = () => {
  ...
  const handleCreate = async () => {
    ...
    setLoading(true);
    try {
      // Agora passamos a duração escolhida
      await couponService.create(newCode, duration); 
      setNewCode('');
      setDuration(60); // Reseta para 1 hora
      await loadCoupons();
    } catch (error) {
  ...
  const toggleLeads = async (code: string) => {
    ...
    // Se ainda não baixou os leads desse cupom, baixa agora
    if (!leadsMap[code]) {
  ...
  // Helper para formatar tempo (ex: 10080 -> "7 dias")
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-1">Crie campanhas e rastreie conversão de leads.</p>
          </div>
