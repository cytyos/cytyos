...
import { Loader2, AlertCircle, X } from 'lucide-react'; // Adicionado o ícone 'X'
...
export const LoginPage = () => {
  ...
  // --- CORREÇÃO AQUI ---
  // Usando a sintaxe compatível com a versão v1 do client do Supabase
    ...
    <div className="min-h-screen bg-[#0f111a] flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-gray-900/50 backdrop-blur-xl border border-gray-800 rounded-3xl p-8 shadow-2xl relative z-10">
        ...
        <button 
            ...
            aria-label="Fechar e voltar para a página inicial"
        >
            <X className="w-5 h-5" />
        </button>

        {/* Header with Cytyos Logo */}
        <div className="flex flex-col items-center mb-8 mt-2">
            {/* QUANDO TIVER O LOGO:
               1. Coloque seu arquivo de imagem (ex: logo.png) na pasta /public do projeto.
               2. Descomente a linha abaixo e ajuste o nome do arquivo.
            */}
            {/* <img src="/logo.png" alt="Cytyos Logo" className="h-16 mb-4 object-contain" /> */}
            
            {/* Placeholder do Logo (Texto com mais destaque) */}
            ...
            {loading ? <Loader2 className="w-5 h-5 animate-spin text-gray-900" /> : (
                ...
                <svg className="w-5 h-5" viewBox="0 0 24 24">
                    <path
                        ...
                        fill="#4285F4"
                    />
                    <pathimport React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { supabase } from '../lib/supabase';
...
export const LoginPage = () => {
  ...
  // Login com Email/Senha (Supabase v2)
  const handleAuth = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    setError('');
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          // Garante que volta para a página certa
          redirectTo: window.location.origin + '/app',
        },
      });
  ...
  return (
    <div className="min-h-screen bg-[#0f111a] flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-gray-900/50 backdrop-blur-xl border border-gray-800 rounded-3xl p-8 shadow-2xl relative z-10">
        ...
        {/* Header */}
        <div className="flex flex-col items-center mb-8 mt-2">
            {/* Lembre-se de adicionar seu logo na pasta public depois */}
