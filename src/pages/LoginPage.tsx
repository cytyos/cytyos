  ...
  const handleGoogleLogin = async () => {
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: window.location.origin + '/app', // Ensure this matches your Redirect URLs in Supabase
        },
      });
      if (error) throw error;
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-[#0f111a] flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-gray-900/50 backdrop-blur-xl border border-gray-800 rounded-3xl p-8 shadow-2xl relative z-10">
        ...
        <div className="flex flex-col items-center mb-8">
            {/* TODO: Replace '/cytyos-logo.png' with your actual logo path inside the public folder.
               I kept the Icon as a fallback or placeholder. 
            */}
            <div className="mb-4">
                {/* Check if you have a logo image, otherwise show the Icon */}
                <div className="p-3 bg-indigo-500/10 rounded-2xl border border-indigo-500/20">
                    <LayoutGrid className="w-8 h-8 text-indigo-400" />
                    {/* <img src="/cytyos-logo.png" alt="Cytyos Logo" className="w-12 h-12 object-contain" /> */}
                </div>
            </div>
        ...
        <button 
            ...
            className="w-full bg-white text-gray-900 hover:bg-gray-100 font-bold py-3 rounded-xl transition-all flex items-center justify-center gap-3 shadow-lg"
        >
            {/* Google SVG Logo */}
