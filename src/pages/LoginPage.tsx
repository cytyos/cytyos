...
export const LoginPage = () => {
  ...
  // --- INICIALIZAÇÃO INTELIGENTE DA ABA ---
  // Se vier da Home clicando em "Começar", abre 'signup'. Se não, 'signin'.
  const [mode, setMode] = useState<AuthMode>(location.state?.tab === 'signup' ? 'signup' : 'signin');
  // ----------------------------------------
      ...
      <div className="w-full max-w-md bg-gray-900/60 backdrop-blur-xl border border-gray-700/50 rounded-3xl p-8 shadow-2xl relative z-10 animate-scale-up">
        ...
        {mode === 'forgot' ? (
          ...
          <>
            ...
            {/* --- GOOGLE AGORA NO TOPO --- */}
            <button type="button" onClick={handleGoogleLogin} disabled={loading} className="w-full bg-white text-gray-900 hover:bg-gray-100 font-bold py-3 rounded-xl transition-all flex items-center justify-center gap-3 shadow-lg mb-6 text-sm">
            ...
            <form onSubmit={handleAuth} className="space-y-4">
                <div>
                    ...
                    <div className="relative mt-1">
                        <Mail className="absolute left-3 top-3 text-gray-500 w-4 h-4" />
                        <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-black/40 border border-gray-600 rounded-xl pl-10 pr-4 py-3 text-white outline-none focus:border-indigo-500 transition-colors text-sm" placeholder={t('auth.email_placeholder')} required />
                    </div>
                </div>
                <div>
