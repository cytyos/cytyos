...
export const PricingModal = ({ isOpen, onClose }: PricingModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-6xl bg-[#0f111a] rounded-3xl border border-indigo-500/30 flex flex-col md:flex-row overflow-hidden shadow-2xl animate-scale-up max-h-[95vh] overflow-y-auto custom-scrollbar z-50">
        ...
        <div className="w-full md:w-5/12 bg-gradient-to-b from-[#0a0c10] to-black p-6 flex flex-col border-r border-white/5 relative">
          ...
          <div className="mb-6 relative z-10">
            ...
            <p className="text-xs text-gray-400 mt-2 border-l-2 border-yellow-500/50 pl-3">
               {t('pricing.warning')}
            </p>
          </div>
          ...
          <div className="space-y-3 flex-1 overflow-y-auto custom-scrollbar pr-2">
                ...
                <ul className="space-y-2">
                    {[1,2,3,4].map(n => (
                        <li key={n} className="flex items-start gap-2 text-[10px] text-gray-400">
                            <Check className="w-3 h-3 text-green-500 shrink-0" /> {t(`pricing.beta_f${n}`)}
                        </li>
                    ))}
                </ul>
            </div>
                ...
                <ul className="space-y-2">
                    {[1,2,3,4].map(n => (
                        <li key={n} className="flex items-start gap-2 text-[10px] text-gray-300">
                            <Rocket className="w-3 h-3 text-indigo-400 shrink-0" /> {t(`pricing.v1_f${n}`)}
                        </li>
                    ))}
                </ul>
            </div>
            ...
            <div className="rounded-xl bg-[#0f111a] border border-purple-500/20 p-4 relative overflow-hidden group hover:border-purple-500/40 transition-colors">
                ...
                <ul className="space-y-2">
                    {[1,2,3,4].map(n => (
                        <li key={n} className="flex items-start gap-2 text-[10px] text-gray-400">
                            <Sparkles className="w-3 h-3 text-purple-500 shrink-0" /> {t(`pricing.v2_f${n}`)}
                        </li>
                    ))}
                </ul>
            </div>
          ...
          <div className="mt-4 pt-4 border-t border-white/10">
            <div className="flex items-center justify-between mb-2">
                 ...
                 <button onClick={() => handleCheckout('pdf')} className="px-3 py-1.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg text-[10px] text-gray-300 transition-colors font-medium">
                    {t('pricing.btn_pdf')}
                </button>
            </div>
          </div>
        ...
        {/* ================= RIGHT SIDE: PRICING SELECTOR ================= */}
        <div className="w-full md:w-7/12 p-8 flex flex-col bg-[#0f111a] relative justify-center">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-6 relative z-10">
              <div className="text-center md:text-left">
                <h4 className="font-bold text-white text-xl mb-1">
                    {billingCycle === 'monthly' ? t('pricing.plan_monthly') : t('pricing.plan_annual')}
                </h4>
                <p className="text-gray-400 text-xs">
                    {billingCycle === 'monthly' ? t('pricing.sub_monthly') : t('pricing.sub_annual')}
                </p>
