...
interface PricingModalProps { isOpen: boolean; onClose: () => void; }
...
export const PricingModal = ({ isOpen, onClose }: PricingModalProps) => {
  ...
  const handleCheckout = (type: 'subscription' | 'pdf') => {
    let baseUrl = type === 'pdf' ? STRIPE_LINKS.pdfOnly : (billingCycle === 'monthly' ? STRIPE_LINKS.monthly : STRIPE_LINKS.yearly);
    if (baseUrl) {
        const checkoutUrl = user?.id ? `${baseUrl}?client_reference_id=${user.id}` : baseUrl;
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-0 md:p-4">
      ...
      <div className="relative w-full max-w-6xl bg-[#0f111a] md:rounded-3xl border border-indigo-500/30 flex flex-col md:flex-row overflow-hidden shadow-2xl animate-scale-up h-[100dvh] md:h-auto md:max-h-[95vh] overflow-y-auto custom-scrollbar z-50">
        <button onClick={(e) => { e.stopPropagation(); onClose(); }} className="fixed top-4 right-4 z-[100] p-2 bg-black/60 hover:bg-white/10 backdrop-blur-md rounded-full text-white border border-white/10 transition-colors shadow-lg cursor-pointer"><X size={24} /></button>
        ...
        <div className="w-full md:w-5/12 bg-gradient-to-b from-[#0a0c10] to-black p-6 md:p-8 flex flex-col border-b md:border-b-0 md:border-r border-white/5 relative shrink-0">
          <div className="mb-6 relative z-10 pt-8 md:pt-0">
            {billingCycle === 'yearly' ? (
                <div className="inline-flex items-center gap-2 mb-3 px-3 py-1 bg-indigo-500/10 border border-indigo-500/20 rounded-full animate-in fade-in slide-in-from-left-2"><Star className="w-3 h-3 text-indigo-400 fill-indigo-400" /><span className="text-[10px] font-bold text-indigo-300 tracking-wider uppercase">{t('pricing.badge')}</span></div>
            ) : (
                <div className="inline-flex items-center gap-2 mb-3 px-3 py-1 bg-emerald-500/10 border border-emerald-500/20 rounded-full animate-in fade-in slide-in-from-left-2"><CheckCircle2 className="w-3 h-3 text-emerald-400" /><span className="text-[10px] font-bold text-emerald-300 tracking-wider uppercase">Beta Access</span></div>
            )}
            <h3 className="text-2xl md:text-3xl font-extrabold text-white leading-tight">{billingCycle === 'yearly' ? t('pricing.title') : t('pricing.beta_title')}</h3>
            <p className="text-xs text-gray-400 mt-3 border-l-2 border-yellow-500/50 pl-3 leading-relaxed">{billingCycle === 'yearly' ? t('pricing.warning') : t('pricing.monthly_warning')}</p>
          </div>
          ...
          <div className="space-y-3 flex-1 overflow-y-auto pr-2 custom-scrollbar">
             ...
             {billingCycle === 'yearly' && (
                 <>
                    ...
                    <div className="rounded-xl bg-[#0f111a] border border-purple-500/20 p-4 relative overflow-hidden group hover:border-purple-500/40 transition-colors animate-in fade-in slide-in-from-bottom-8">
                        ...
                        <ul className="space-y-2">{[1,2,3,4].map(n => (<li key={n} className="flex items-start gap-2 text-[10px] text-gray-400"><Sparkles className="w-3 h-3 text-purple-500 shrink-0 mt-0.5" /> <span>{t(`pricing.v2_f${n}`)}</span></li>))}</ul>
                    </div>
                 </>
             )}
             ...
             {billingCycle === 'monthly' && (
                 <div className="p-4 border border-dashed border-gray-700 rounded-xl text-center bg-gray-900/20">
                     <p className="text-xs text-gray-500 leading-relaxed">{t('pricing.monthly_warning')} <span className="text-indigo-400 font-bold block mt-1">{t('pricing.monthly_warning_highlight')}</span></p>
                 </div>
             )}
          </div>
          ...
          <div className="mt-6 pt-6 border-t border-white/10">
            <div className="flex items-center justify-between mb-2">
              <div className="text-[10px] text-gray-400"><strong className="text-white block">{t('pricing.small_title')}</strong>{t('pricing.small_desc')}</div>
              <button onClick={() => handleCheckout('pdf')} className="px-3 py-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg text-[10px] text-gray-300 transition-colors font-medium whitespace-nowrap">{t('pricing.btn_pdf')}</button>
            </div>
          </div>
        </div>
        ...
        <div className="w-full md:w-7/12 p-6 md:p-8 flex flex-col bg-[#0f111a] relative justify-center shrink-0">
          <div className="text-center mb-8">
            ...
            <div className="grid grid-cols-2 bg-black p-1 rounded-full border border-white/10 relative w-full max-w-[350px] mx-auto">
              ...
              <button onClick={() => setBillingCycle('yearly')} className={`relative z-10 py-3 rounded-full text-xs font-bold transition-all flex items-center justify-center gap-2 ${billingCycle === 'yearly' ? 'text-white' : 'text-gray-500'}`}>{t('pricing.yearly')} <span className="bg-green-500 text-black text-[9px] px-1.5 py-0.5 rounded font-extrabold">{t('pricing.save_pct')}</span></button>
          ...
          <div className="mt-auto bg-black/40 rounded-xl p-4 border border-white/5 flex flex-col gap-3 pb-8 md:pb-4">
            <div className="flex items-center justify-between"><p className="text-xs text-gray-400 font-medium flex items-center gap-2"><Lock className="w-3 h-3 text-emerald-500"/> {t('pricing.coupon_label')}</p></div>
            <div className="flex gap-2">
              ...
              <button onClick={handleValidateKey} disabled={isValidating || !accessKey} className="px-4 py-2 bg-indigo-600/20 text-indigo-300 border border-indigo-500/30 hover:bg-indigo-600 hover:text-white font-bold rounded-lg text-xs transition-colors disabled:opacity-50 flex items-center gap-2">{isValidating ? <Loader2 className="w-3 h-3 animate-spin" /> : t('pricing.validate')}</button>
            </div>
