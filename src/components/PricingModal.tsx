...
export const PricingModal = ({ isOpen, onClose }: PricingModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-0 md:p-4">
      {/* Backdrop */}
      <div className="absolute inset-0 bg-black/95 backdrop-blur-xl transition-opacity animate-fade-in" onClick={handleClose} />

      {/* Main Container: Full height on mobile (dvh), rounded on desktop */}
          ...
          <div className="text-center mb-8">
            ...
            <div className="grid grid-cols-2 bg-black p-1 rounded-full border border-white/10 relative w-full max-w-[350px] mx-auto">
                <button 
                    ...
                    className={`relative z-10 py-3 rounded-full text-xs font-bold transition-all flex items-center justify-center ${billingCycle === 'monthly' ? 'text-white' : 'text-gray-500 hover:text-gray-300'}`}
                >
                    {t('pricing.monthly')}
                </button>
                <button 
                    onClick={() => setBillingCycle('yearly')} 
                    className={`relative z-10 py-3 rounded-full text-xs font-bold transition-all flex items-center justify-center gap-2 ${billingCycle === 'yearly' ? 'text-white' : 'text-gray-500 hover:text-gray-300'}`}
                >
                    {t('pricing.yearly')} 
          ...
          <div className="mt-auto bg-black/40 rounded-xl p-4 border border-white/5 flex flex-col gap-3 pb-8 md:pb-4">
             ...
             <div className="flex gap-2">
                <input 
                    ...
                    className="flex-1 bg-black/50 border border-white/10 rounded-lg px-3 py-3 md:py-2 text-sm text-white outline-none focus:border-indigo-500 uppercase transition-all placeholder:text-gray-700" 
