...
export const MapControls = () => {
  ...
  const handleDrawToggle = () => { if (drawMode === 'draw_polygon') { setDrawMode('simple_select'); } else { setDrawMode('draw_polygon'); if (is3D) setIs3D(false); } };
  ...
  return (
    <div className="w-full h-full pointer-events-none flex flex-col justify-between">
      ...
      {/* 1. BARRA DE FERRAMENTAS E BUSCA (TOPO) */}
      <div className="pointer-events-auto w-full max-w-md mx-auto relative z-50 p-2 flex flex-col gap-2" ref={searchContainerRef}>
        ...
        <div className="flex gap-2">
            ...
            <div className="bg-[#0f111a]/90 backdrop-blur-md border border-white/10 rounded-xl p-1 flex items-center shadow-xl">
                <button onClick={() => setMeasurementSystem('metric')} className={`px-3 py-1.5 h-full rounded-lg text-[10px] font-bold transition-all ${measurementSystem === 'metric' ? 'bg-indigo-600 text-white shadow-sm' : 'text-gray-400 hover:text-white'}`}>M</button>
                <div className="w-px h-3 bg-white/10 mx-1"></div>
                <button onClick={() => setMeasurementSystem('imperial')} className={`px-3 py-1.5 h-full rounded-lg text-[10px] font-bold transition-all ${measurementSystem === 'imperial' ? 'bg-indigo-600 text-white shadow-sm' : 'text-gray-400 hover:text-white'}`}>FT</button>
            </div>
        </div>

        {/* Botões de Ferramentas (Restaurados) */}
        ...
        {hasDrawing && showClearConfirm && (
            <div className="bg-[#0f111a] border border-red-500/50 rounded-xl p-2 shadow-2xl flex items-center justify-between gap-2 animate-fade-in">
                <span className="text-[10px] text-white font-bold pl-1">{t('map.delete_confirm_title')}</span>
                <div className="flex gap-1">
                    <button onClick={handleConfirmClear} className="bg-red-500/20 hover:bg-red-500 text-red-400 hover:text-white p-1 rounded-lg transition-colors"><Check className="w-3 h-3" /></button>
                    <button onClick={() => setShowClearConfirm(false)} className="bg-gray-800 hover:bg-gray-700 text-gray-400 hover:text-white p-1 rounded-lg transition-colors"><X className="w-3 h-3" /></button>
                </div>
            </div>
        )}
      </div>
