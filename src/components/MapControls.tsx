...
export const MapControls = () => {
  ...
  const handleDrawToggle = () => {
      if (drawMode === 'draw_polygon') { setDrawMode('simple_select'); } 
      else { setDrawMode('draw_polygon'); if (is3D) setIs3D(false); }
  };
                    ...
                    <div className="absolute top-full mt-2 w-full bg-[#0f111a] border border-gray-700 rounded-xl shadow-2xl overflow-hidden max-h-60 overflow-y-auto custom-scrollbar z-[100]">
                        <ul>
                            {searchResults.map((result) => (
                                <li key={result.id} onMouseDown={(e) => { e.preventDefault(); handleSelectLocation(result); }} className="px-4 py-3 hover:bg-gray-800 cursor-pointer border-b border-gray-800 last:border-0 transition-colors group">
