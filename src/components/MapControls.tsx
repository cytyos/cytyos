...
export const MapControls = () => {
  ...
  // UX: Ao desenhar, forçar modo 2D para facilitar
  const toggleDraw = () => {
  ...
  // --- CORREÇÃO: pointer-events-auto no container ---
  return (
    <div className="flex flex-col gap-3 w-[90vw] max-w-md mx-auto pointer-events-auto relative">
      ...
      <div className="flex gap-2 w-full relative z-50" ref={searchContainerRef}>
        <div className="flex-1 relative group">
            ...
            />
            {searchValue && (
                <button onClick={clearSearch} className="absolute inset-y-0 right-0 pr-3 flex items-center text-gray-500 hover:text-white"><X className="h-3 w-3" /></button>
            )}
                ...
                <div className="absolute bottom-full mb-2 w-full bg-[#0f111a] border border-gray-700 rounded-xl shadow-2xl overflow-hidden max-h-60 overflow-y-auto custom-scrollbar z-50">
                    <ul>
                        {searchResults.map((result) => (
                            <li key={result.id} onClick={() => handleSelectLocation(result)} className="px-4 py-3 hover:bg-gray-800 cursor-pointer border-b border-gray-800 last:border-0 transition-colors">
                                <div className="text-xs font-bold text-white truncate">{result.place_name.split(',')[0]}</div>
      ...
      <div className="bg-[#0f111a]/90 backdrop-blur-md border border-white/10 rounded-2xl p-1.5 flex justify-between items-center shadow-2xl relative z-40">
        ...
        <button onClick={toggleDraw} className={`flex flex-col items-center justify-center w-14 py-1.5 rounded-xl transition-all duration-200 group relative ${drawMode === 'draw_polygon' ? 'bg-indigo-500 text-white shadow-[0_0_15px_rgba(99,102,241,0.5)] scale-105' : 'hover:bg-white/5 text-gray-400 hover:text-white'}`}>
